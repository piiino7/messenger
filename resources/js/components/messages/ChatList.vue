<template>
    <div class="container py-8">
        ...
        <div class="card-list" id="chat-list">
            <a v-for="conversation in $root.conversations" v-if="$root.newChat == null" v-bind:key="conversation.id" v-bind:href="'#' + conversation.id" @click.prevent="setConversation(conversation)" class="card border-0 text-reset">
                <div class="card-body">
                    <div class="row gx-5">
                        <div class="col-auto">
                            <div class="avatar" :class = "{'avatar-online': conversation.participants[0].isOnline}">
                                <img v-bind:src="conversation.participants[0].avatar_url" class="avatar-img" alt="" srcset="">
                            </div>
                        </div>
                        ...
                        <div class="col">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="line-clamp me-auto">
                                    {{conversation.last_message.type == 'attachment'? 'File' : conversation.last_message.body }}
                                </div>
...
export default {
    ...
    methods: {
        setConversation(conversation) {
            ...
            //this.$root.onlineTime = moment(this.$root.conversation.participants[0].online).fromNow();
            this.$root.onlineTime = moment(this.$root.conversation.participants[0].online).format('MMMM Do YYYY, h:mm:ss a');
